...
from coor_main_new import evaluate
...
def parse_args():
    ...
    parser.add_argument(
        '--OODDataset', default='QT+KW+KO',
        choices=["QT","KW", "KWP", "QT+KW","KO","KOP","QT+KO","KW+KO","QT+KW+KO"],
        help="Run on VQA-2.0 instead of VQA-CP 2.0"
    )
    parser.add_argument(
    ...
    parser.add_argument('--output', type=str, default='logs/vqavs01')
...
def main():
    ...
    eval_dset = VQAFeatureDataset(OODDataset, dictionary, dataset=dataset
                                  )
    ...
    # 加载权重模型
    model_state = torch.load('/home/zc/zc/Test/model.pth')
    ...
    model.eval()
    results = evaluate(args,model, eval_loader, qid2type)
    ...
    yesno_answer_right = results['yesno_answer_right']
    number_answer_right = results['number_answer_right']
    other_answer_right = results['other_answer_right']
    ...
    print('\tyn score: %.2f other score: %.2f num score: %.2f\n' % (100 * yn, 100 * other, 100 * num))
    print('\tyn 准确率: %.2f other 准确率: %.2f num 准确率: %.2f\n' % (100 * yesno_answer_right, 100 * other_answer_right, 100 * number_answer_right))
    print('\tCGR: %.2f\n' % (100 * right_att_score))
    print('\tCGW: %.2f\n' % (100 * wrong_att_score))
    print('\tCGD: %.2f\n' % (100 * (right_att_score - wrong_att_score)))
