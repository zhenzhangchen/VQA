...
class NewGSPlugin():
    def __init__(self, gs_flag=True):
        super().__init__()
        dtype = torch.cuda.FloatTensor  # 使用 GPU
        with torch.no_grad():
            self.Pl_v = torch.autograd.Variable(torch.eye(1024).type(dtype))  # 视觉投影矩阵
            self.Pl_q = torch.autograd.Variable(torch.eye(1024).type(dtype))  # 文本投影矩阵
        self.exp_count = 0  # 计数器
    @torch.no_grad()
    def before_update(self, model, before_batch_input, batch_index, len_dataloader, train_exp_counter, is_video):
            ...
            for n, w in model.named_parameters():
                if n == "weights":  # 扩展到融合层
                    if is_video:
                        ...
                        k = torch.mm(self.Pl_v, torch.t(r))
                        self.Pl_v = torch.sub(self.Pl_v, torch.mm(k, torch.t(k)) / (alpha + torch.mm(k, r)))
                        self.Pl_v.data = self.Pl_v.data / torch.norm(self.Pl_v.data, p='fro')  # 范数约束
                        w.grad.data = torch.mm(w.grad.data, torch.t(self.Pl_v.data))
                    else:
                        r = torch.mean(before_batch_input, 0, True)
                        k = torch.mm(self.Pl_q, torch.t(r))
                        self.Pl_q = torch.sub(self.Pl_q, torch.mm(k, torch.t(k)) / (alpha + torch.mm(k, r)))
                        self.Pl_q.data = self.Pl_q.data / torch.norm(self.Pl_q.data, p='fro')  # 范数约束
                        w.grad.data = torch.mm(w.grad.data, torch.t(self.Pl_q.data))  
...
class GSPlugin():
    def __init__(self, gs_flag = True):
        ...
        with torch.no_grad():
            ...
            self.Pl = torch.autograd.Variable(torch.eye(1024).type(dtype))
        self.exp_count = 0
    ...
    def before_update(self, model, before_batch_input, batch_index, len_dataloader, train_exp_counter,is_video):
        ...
        if train_exp_counter != 0:
            for n, w in model.named_parameters():
                if n == "weight":
                    r = torch.mean(before_batch_input, 0, True) #【1,1024】
                    k = torch.mm(self.Pl, torch.t(r)) #【1024,1】
                    self.Pl = torch.sub(self.Pl, torch.mm(k, torch.t(k)) / (alpha + torch.mm(k, r))) #【1024,1024】

                    pnorm2 = torch.norm(self.Pl.data, p='fro') #【1024,1024】

                    self.Pl.data = self.Pl.data / pnorm2 #【1024,1024】
