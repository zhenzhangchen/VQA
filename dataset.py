...
def _load_dataset(dataroot, name, img_id2val, dataset):
    ...
    if dataset == 'cpv2':
        ...
        if name == 'val' or name == 'train':
            answer_path = os.path.join(dataroot, 'VQAVS', '%s_vs_target.pkl' % name)
            question_path = os.path.join(dataroot, 'VQAVS', '%s-Ques.json' % name)
        else :
            answer_path = os.path.join(dataroot, name, '%s_vs_target.pkl' % name)
            question_path = os.path.join(dataroot, name, '%s-Ques.json' % name)
        with open(question_path) as f:
            questions = json.load(f)
        with open(answer_path, 'rb') as f:
            answers = cPickle.load(f)
    ...
    # sort = dict(sorted(img_id2val.items()))
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', dataset='cpv2',
        ...
        self.name = name
        if dataset == 'cpv2'  or dataset == 'vqavs':
        ...
        # with open('util/qid2type_%s_trainval.json' % dataset, 'r') as f:
        #     self.qid2type = json.load(f)
        ...
        if use_hdf5:
            ...
            self.spatial = {}
            self.image_id2ix = {}
            h5_path_train = os.path.join('util', 'train36.hdf5')
            ...
            h5_path_test = os.path.join('util', 'val36.hdf5')
            ...
            self.image_id2ix['train'] = pickle.load(
                open(os.path.join('util',  f'train36_imgid2img.pkl'), 'rb'))
            self.image_id2ix['val'] = pickle.load(
                open(os.path.join('util',  f'val36_imgid2img.pkl'), 'rb'))
        else:
            self.image_id2ix = None
        # self.
        self.entries = _load_dataset(dataroot, name, self.image_id2ix, dataset=dataset)
        # if name == 'train':
        #     self.entries_test = _load_dataset(dataroot, 'val', self.image_id2ix, dataset=dataset)
        #     self.entries += self.entries_test[:15000]
    ...
    def tensorize(self):
        ...
        for entry in tqdm(self.entries, ncols=100, desc="tensorize"):
            ...
            # 统计数据集中what color类型答案种类数
        #     if entry['answer']['question_type'] == 'what color':
        #         if   entry['answer']['labels'] is not None:
        #           for i in entry['answer']['labels']:  
        #               if self.label2ans[i] not in mydict:  
        #                   mydict[self.label2ans[i]] = 1  
        #               elif self.label2ans[i] in mydict:  
        #                   mydict[self.label2ans[i]] += 1 
        ...
        # print(sorted_mydict)    
            ...
            # if entry['answer']['question_type'] == 'what color':
            #     if entry['answer']['labels'] is not None:
            #        print(entry['image_id'],entry['question']) 
                

              #统计某张图片所有问题和答案以及对应分数 
            # if(entry['image_id']==969):
            #     print(entry['question'])
            #     j=0
            #     for i in entry['answer']['labels']:
            #         print(self.label2ans[i]) 
            #         print(entry['answer']['scores'][j])
            #         j+=1
    ...
    def __getitem__(self, index):
        ...
        if self.image_to_fe is not None:
            ...
            spatial = np.array(self.spatial[split][entry['img_idx']])
        ...
        # if self.qid2type[str(q_id)] == 'number':
        #     ans_type = 1.
        # else:
        #     ans_type = 3.
        ...
        if self.use_hdf5:
            if self.name == 'train':
                ...
                if str(q_id) in self.train_hintscore.keys():
                    train_hint = torch.tensor(self.train_hintscore[str(q_id)]).float()
                elif str(q_id) in self.test_hintscore.keys():
                    train_hint = torch.tensor(self.test_hintscore[str(q_id)]).float()
                else:
                    train_hint = torch.ones((36)) / 36
                # if str(q_id) in self.train_hintscore.keys() or str(q_id) in self.test_hintscore.keys():
                #     type_mask = torch.tensor(self.type_mask[str(q_id)])
                #     notype_mask = torch.tensor(self.notype_mask[str(q_id)])
                # else:
                #     type_mask = ques_mask
                #     notype_mask = ques_mask
                if "bias" in entry:
                    # return features, spatial, ques, target, entry["bias"], train_hint, type_mask, notype_mask, ques_mask
                    return features, spatial, ques, target, entry["bias"],q_id
                else:
                    # print(entry['question'])
                    # return features, ques, target, 0, q_id, train_hint
                    return features, spatial, ques, target, 0
                    # return features, spatial, ques, target, 0, q_id, train_hint, ques_mask, type_mask
            else:
                # test_hint=torch.tensor(self.test_hintscore[str(q_id)])
                if str(q_id) in self.test_hintscore.keys():
                    test_hint = torch.tensor(self.test_hintscore[str(q_id)]).float()
                elif str(q_id) in self.train_hintscore.keys():
                    test_hint = torch.tensor(self.train_hintscore[str(q_id)]).float()
                else:
                    test_hint = torch.zeros((36)) / 36
                if "bias" in entry:
                    return features, spatial, ques, target, entry["bias"], q_id,test_hint
                else:
                    return features, spatial, ques, target, 0, q_id, test_hint, ques_mask
        else:
            if self.name == 'train':
                train_hint = torch.tensor(self.train_hintscore[str(q_id)])
                type_mask = torch.tensor(self.type_mask[str(q_id)])
...
class VQAFeatureDatasetClip(Dataset):
    def __init__(self, name, dictionary,preprocess,dataroot='data', dataset='cpv2',
                 use_hdf5=True, cache_image_features=False):
        super(VQAFeatureDatasetClip, self).__init__()
        self.name = name
    ...
    def __getitem__(self, index):
        ...
        features = os.path.join('data/',f'{self.name}2014', f'COCO_{self.name}2014_{img_id:012d}.jpg')

        try:  
            image = self.preprocess(Image.open(features))  
        except FileNotFoundError:
            if self.name=="train":  
                features = os.path.join('data/', 'val2014', f'COCO_val2014_{img_id:012d}.jpg')  
            # print(f"File not found. Attempting with new name {features}.")    
            else:
                features = os.path.join('data/', 'train2014', f'COCO_train2014_{img_id:012d}.jpg')                
            image = self.preprocess(Image.open(features))

        question = clip.tokenize(entry['question'])
        ...
        if self.use_hdf5:
            if self.name == 'train':
                ...
                if "bias" in entry:
                    # return features, spatial, ques, target, entry["bias"], train_hint, type_mask, notype_mask, ques_mask
                    return image, question, target, entry["bias"]
                else:
                    # print(entry['question'])
                    # return features, ques, target, 0, q_id, train_hint
                ...
                if str(q_id) in self.test_hintscore.keys():
                    ...
                    test_hint = torch.zeros((36)) / 36
                if "bias" in entry:
                    return image, question, target, entry["bias"],q_id,test_hint
                else:
                    return features, spatial, ques, target, 0, q_id, test_hint, ques_mask
        else:
            if self.name == 'train':
...
if __name__ == "__main__":
    ...
    args = parse_args()
    dictionary = Dictionary.load_from_file("/home/lqw/lqw_pytorch/CSS-VQA/data/dictionary.pkl")

    train_dset = VQAFeatureDataset('train', dictionary, dataset="cpv2",
