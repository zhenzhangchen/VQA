...
def train(model, train_loader, eval_loader, args, qid2type):
    ...
    output = args.output
    optim = torch.optim.Adamax(model.parameters(), lr=0.0015)
    ...
    for epoch in range(num_epochs):
        ...
        t = time.time()
        # [512,36,2048],[512,36,7],[512,14],[512,2274],[512,2274],[512,36],[512,14],[512,14],[512,14]
        # for i, (v, s, q, a, b, hintscore, type_mask, notype_mask, q_mask) in tqdm(enumerate(train_loader), ncols=100,
        #                                                                           desc="Epoch %d" % (epoch + 1),
        #                                                                           total=len(train_loader)):
        for i, (v, s, q, a, b, qid) in tqdm(enumerate(train_loader), ncols=100,
                                       desc="Epoch %d" % (epoch + 1),
            ...
            optim.zero_grad()
            if mode == 'updn':
                q_coor, v_coor = model(v, s, q, a, b, None, coor_data, 'first')
                out_q = model.fusion_module.fc_out(q_coor) #[batch,1024]->[batch,2274]
                loss_q = F.binary_cross_entropy_with_logits(out_q, a)
                loss_q.backward(retain_graph=True)
                
                gs_plugin.before_update(model.fusion_module.fc_out,q_coor, 
                                        i, len_dataloader, gs_plugin.exp_count)
                optim.step()
                optim.zero_grad()
                gs_plugin.exp_count += 1
                
                out_v = model.fusion_module.fc_out(v_coor.detach()) #[batch,1024]->[batch,2274]
                loss_v = F.binary_cross_entropy_with_logits(out_v, a)
                loss_v.backward(retain_graph=True)
                
                gs_plugin.before_update(model.fusion_module.fc_out, v_coor, 
                                        i, len_dataloader, gs_plugin.exp_count)
                optim.step()
                optim.zero_grad()
                gs_plugin.exp_count += 1
                # TODO 重新计算loss_q,loss_v
                loss_q, loss_v = model(v, s, q, a, b, None, coor_data, 'second')
                ...
                pred, loss, _ = model(v, s, q, a, b, None, coor_data, 'third')
                if (loss != loss).any():
                ...
                if random_num <= qvp:
                    # first train
                    # pred, loss, word_emb, loss_q, loss_v = model(v, s, q, a, b, None, None, 'first')
                    pred, loss, word_emb, att, att_new, loss_att_2 = model(v, s, q, a, b, None)
                    word_grad = torch.autograd.grad((pred * (a > 0).float()).sum(), word_emb, create_graph=True)[0]
                    ...
                    # pred, _, _ = model(v, s, q2, None, b, None, coor_data, 'second')
                    pred, _, _ = model(v, s, q2, None, b, None)
                    ...
                    # pred, loss, _ = model(v, s, q3, a2, b, None, coor_data, 'second')
                    pred, loss, _, att, att_new, loss_att_2 = model(v, s, q3, a2, b, None)
                    ...
                    # first train
                    # pred, loss, _, loss_q, loss_v = model(v, s, q, a, b, None, None, 'first')
                    pred, loss, _, att, att_new, loss_att_2 = model(v, s, q, a, b, None)
                    visual_grad = torch.autograd.grad((pred * (a > 0).float()).sum(), v, create_graph=True)[0]
                    ...
                    if topv == -1:
                        ...
                        v_mask_ind = v_grad_mask * v_ind
                        for x in range(a.shape[0]):
                            num = len(torch.nonzero(v_grad_mask[x - 1]))
                            v_mask[x].scatter_(0, v_mask_ind[x, :num], 1)
                    else:
                        v_grad_ind = v_grad.sort(1, descending=True)[1][:, :topv]
                        v_star = v_ind.gather(1, v_grad_ind)
                    ...
                    # pred, _, _ = model(v, s, q, None, b, v_mask, coor_data, 'second')
                    pred, _, _ = model(v, s, q, None, b, v_mask)
                    pred_ind = torch.argsort(pred, 1, descending=True)[:, :5]
                    false_ans = torch.ones(pred.shape[0], pred.shape[1]).cuda()
                    false_ans.scatter_(1, pred_ind, 0)
                    a2 = a * false_ans
                    ...
                    # pred, loss, _ = model(v, s, q, a2, b, v_mask, coor_data, 'second')
                    pred, loss, _, att, att_new, loss_att_2 = model(v, s, q, a2, b, v_mask)
                    ...
                    optim.step()
                    optim.zero_grad()
                    # print(loss_att, loss_att_2)

                    total_loss += loss.item() * q.size(0)
                # print(att, att_new)
        # logger.write(str(att))
        # logger.write(att_new)
        ...
        logger.write('\tbest_eval score: %.2f' % (100 * best_eval_score))
        # model_path_epoch = os.path.join(output, 'model_' + str(epoch) + '.pth')
...
def evaluate(model, dataloader, qid2type):
    ...
    for v, s, q, a, b, qids, _ in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
        v = Variable(v, requires_grad=False).cuda()
        s = Variable(s, requires_grad=False).cuda()
        q = Variable(q, requires_grad=False).cuda()
        pred, _, _ = model(v, s, q, None, None, None, None, 'third')
        # pred, _, _ = model(v, s, q, None, None, None)
